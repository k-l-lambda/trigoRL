...
class TGNByteTokenizer:
    ...
    def encode_batch(
        ...
        **kwargs
        ...
        """
        return torch.stack([
            self.encode(text, max_length=max_length, **kwargs)
            for text in texts
        ])
    ...
    def decode(
        ...
        if skip_special_tokens:
            byte_tokens = [
                ...
                if t < self.BYTE_RANGE  # Only keep standard bytes
            ]
        else:
            byte_tokens = [
                t for t in token_list
                if t < self.BYTE_RANGE or t in [self.START_TOKEN_ID, self.END_TOKEN_ID]
            ]
